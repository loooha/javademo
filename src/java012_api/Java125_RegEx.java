...
public class Java125_RegEx {
	...
	public static void main(String[] args) {
		...
		// a-zA-Z_*$가 sn변수에 포함이 되어있으면 true, 아니면 false를 리턴한다.
		System.out.println(sn.matches(".*[a-zA-Z_*$].*"));

		// sn변수에 숫자 0-9가 포함되어있으면 true, 아니면 false를 리턴한다.
		System.out.println(sn.matches(".*[0-9].*"));
		System.out.println(sn.matches(".*[\\d].*"));
		System.out.println(sn.matches(".*\\d.*"));

		// sn변수에 숫자 0-9가 포함되어있지 않으면 true, 아니면 false를 리턴한다.
		System.out.println(sn.matches(".*[^0-9].*"));
		System.out.println(sn.matches(".*[\\D].*"));
		...
		// st변수에 저장된 값이 a이거나 b로 시작하고 임의문자가 5개이면 true, 아니면 false를 리턴한다.
		System.out.println(st.matches("[ab].{5}"));

		// st변수에 저장된 값이 a이거나 b로 시작하고 임의문자가 1~5개이며 cc로 끝나면 true, 아니면 false로 리턴한다.
		st = "aetwefcc";
		...
		// Pattern.matcher(): 빠르게 text(String)이 주어진 Pattern에 있는지를 확인한다.
		// Pattren.comfile(): 여러개의 텍스트를 재사용 가능한 Pattern 인스턴스로 컴파일한다.
		...
		// end(int group): 지정된 그룹이 매칭되는 한칸 뒤 인덱스 반환
		// group(): 최초로 그룹 매칭된 패던을 반환
		// group(int group): 그룹 매칭된 부분 중 group번째 패턴을 반환
		// groupCount(): 패턴 내 그룹핑(괄호로 묶은 패턴들)의 갯수 반환
